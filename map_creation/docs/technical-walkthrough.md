# Technical walkthrough for the agent

## 1. Prepare the plan

Run the optional [offline helper demo](offline-demo.md). Inspect `previews/guide.jpg`, then replace the example assumptions in `planning/brief.md` and `planning/societies.md`. Select or trial an art style, save its reference in `assets/masters/`, and update `style.json`.
